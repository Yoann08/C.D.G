 <?php 
  require_once '../../C.D.G/config/connectBDD.php';
  include '../../C.D.G/include/head.php';
 ?>

<body>
<!-- ---------------NAVBAR--------------- -->

<?php
    include '../../C.D.G/include/navbar.php';
?>

<!-- ---------------CAROUSEL--------------- -->

<body>
  <swiper-container class="mySwiper" pagination="true" effect="coverflow" grab-cursor="true" centered-slides="true"
    slides-per-view="auto" coverflow-effect-rotate="50" coverflow-effect-stretch="0" coverflow-effect-depth="100"
    coverflow-effect-modifier="1" coverflow-effect-slide-shadows="true">
    <swiper-slide>
      <img class="rounded-2xl shadow-md shadow-purple-dark/[.7]" src="../../C.D.G/assets/image/2.png" />
    </swiper-slide>
    <swiper-slide>
      <img class="rounded-2xl shadow-md shadow-purple-dark/[.7]" src="../../C.D.G/assets/image/3.png" />
    </swiper-slide>
    <swiper-slide>
      <img class="rounded-2xl shadow-md shadow-purple-dark/[.7]" src="../../C.D.G/assets/image/4.png" />
    </swiper-slide>
    <swiper-slide>
      <img class="rounded-2xl shadow-md shadow-purple-dark/[.7]" src="../../C.D.G/assets/image/5.png" />
    </swiper-slide>
    <swiper-slide>
      <img class="rounded-2xl shadow-md shadow-purple-dark/[.7]" src="../../C.D.G/assets/image/2.png" />
    </swiper-slide>
    <swiper-slide>
      <img class="rounded-2xl shadow-md shadow-purple-dark/[.7]" src="../../C.D.G/assets/image/4.png" />
    </swiper-slide>
    <swiper-slide>
      <img class="rounded-2xl shadow-md shadow-purple-dark/[.7]" src="../../C.D.G/assets/image/3.png" />
    </swiper-slide>
    <swiper-slide>
      <img class="rounded-2xl shadow-md shadow-purple-dark/[.7]" src="../../C.D.G/assets/image/5.png" />
    </swiper-slide>
    <swiper-slide>
      <img class="rounded-2xl shadow-md shadow-purple-dark/[.7]" src="../../C.D.G/assets/image/2.png" />
    </swiper-slide>
  </swiper-container>

  <script src="https://cdn.jsdelivr.net/npm/swiper@9/swiper-element-bundle.min.js"></script>

<!-- ---------------SECTION 1--------------- -->

<img class= "mt-16 shadow-md shadow-purple-dark/[.7]" src="../../C.D.G/assets/image/section1.png" />

<!-- ---------------SECTION HTML--------------- -->

<div class="py-16 shadow-md shadow-purple-dark/[.7]">
    <div class="flex flex-row justify-between mr-8 sm:mr-20 sm:ml-14 md:mr-32 md:ml-24">
        <img class="w-28" src="../../C.D.G/assets/image/html.png" />
        <a href="produits.php" type="submit" class="btn flex justify-around w-auto h-10 text-purple-light uppercase border-2 font-medium rounded-2xl text-sm px-2 py-2 text-center shadow-sm shadow-purple-dark">Voir plus</a>
    </div>
    <div class="flex flex-wrap justify-center gap-5 md:mx-16 2xl:gap-28">
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    </div>
</div>

<!-- ---------------SECTION CSS--------------- -->

<div class="bg-purple-dark/[.07] py-16 shadow-md shadow-purple-dark/[.7]">
    <div class="flex flex-row justify-between mr-8 sm:mr-20 sm:ml-14 md:mr-32 md:ml-24">
        <img class="w-28" src="../../C.D.G/assets/image/css.png" />
        <a href="produits.php" type="submit" class="btn flex justify-around w-auto h-10 text-purple-light uppercase border-2 font-medium rounded-2xl text-sm px-2 py-2 text-center shadow-sm shadow-purple-dark">Voir plus</a>
    </div>
    <div class="flex flex-wrap justify-center gap-5 md:mx-16 2xl:gap-28">
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    </div>
</div>

<!-- ---------------SECTION JS--------------- -->

<div class="py-16 shadow-md shadow-purple-dark/[.7]">
    <div class="flex flex-row justify-between mr-8 sm:mr-20 sm:ml-14 md:mr-32 md:ml-24">
        <img class="w-28" src="../../C.D.G/assets/image/js.png" />
        <a href="produits.php" type="submit" class="btn flex justify-around w-auto h-10 text-purple-light uppercase border-2 font-medium rounded-2xl text-sm px-2 py-2 text-center shadow-sm shadow-purple-dark">Voir plus</a>
    </div>
    <div class="flex flex-wrap justify-center gap-5 md:mx-16 2xl:gap-28">
        <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
        <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
        <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
        <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    </div>
</div>

<!-- ---------------SECTION PHP--------------- -->
<div class="bg-purple-dark/[.07] py-16 shadow-md shadow-purple-dark/[.7]">
    <div class="flex flex-row justify-between mr-8 sm:mr-20 sm:ml-14 md:mr-32 md:ml-24">
        <img class="w-28" src="../../C.D.G/assets/image/php.png" />
        <a href="produits.php" type="submit" class="btn flex justify-around w-auto h-10 text-purple-light uppercase border-2 font-medium rounded-2xl text-sm px-2 py-2 text-center shadow-sm shadow-purple-dark">Voir plus</a>
    </div>
    <div class="flex flex-wrap justify-center gap-5 md:mx-16 2xl:gap-28">
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    <a href="produit.php" class=" mt-16 h-72 w-60 block p-6 bg-white border border-purple-light border-gray-200 rounded-2xl drop-shadow-md shadow-md shadow-purple-dark/[.7]"></a>
    </div>
</div>

<!-- ---------------BACK TO TOP--------------- -->

<!-- <button id="to-top-button" onclick="goToTop()" title="Go To Top"
    class="fixed z-50 bottom-10 right-10 p-4 border-0 w-14 h-14 rounded-full shadow-md bg-purple-light hover:bg-purple-700 text-white text-lg font-semibold transition-colors duration-300">
    <svg src="../../C.D.G/assets/image/backtotop.png" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
        <path d="M12 4l8 8h-6v8h-4v-8H4l8-8z" />
    </svg>
    <span class="sr-only">Go to top</span>
</button> -->

<!-- ---------------FOOTER--------------- -->

<?php
    include '../../C.D.G/include/footer.php';
?>

    <script src="https://unpkg.com/@themesberg/flowbite@latest/dist/flowbite.bundle.js"></script>
    <script defer src="https://use.fontawesome.com/releases/v5.15.4/js/all.js" integrity="sha384-rOA1PnstxnOBLzCLMcre8ybwbTmemjzdNlILg8O7z1lUkLXozs4DHonlDtnE7fpc" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/flowbite/1.6.5/flowbite.min.js"></script>
</body>
</html>